  ...
  remove_ node@(Node n) removeFromParent = do
    ...
    case k `compare` key of
      ...
      EQ -> case (left, right) of
        ...
        (Node left', Node right') -> if useSuccessor
          then readTVar right' >>= \case
...
getLT t k = get_ =<< readTVar t
 where
  ...
  get_ (Node n) = do
    ...
    if k' < k then ((k', v) :) <$> nodeToList left else return []
...
getGT t k = get_ =<< readTVar t
 where
  ...
  get_ (Node n) = do
    Tree k' v left right <- readTVar n
    if k' > k then ((k', v) :) <$> nodeToList right else return []
...
depth t = depth_ 0 =<< readTVar t
 where
  depth_ acc Empty    = return acc
  depth_ acc (Node n) = do
    Tree _ _ left right <- readTVar n
    sl                  <- depth_ (acc + 1) left
    sr                  <- depth_ (acc + 1) right
    return $ max sl sr
...
dump t = go "" =<< readTVar t
 where
  ...
  go indent (Node n) = do
    ...
    dl                  <- go (indent ++ "  ") left
    dr                  <- go (indent ++ "  ") right
    return $ show k ++ " -> " ++ show v ++ fl ++ dl ++ fr ++ dr
