...
module Tests.BalancedIndexSpec where
...
import Data.IxSet.BalancedIndex as M

import System.IO (hPutStr, stderr, stdout)

qcStmBalacedIndexProps = testGroup "Balanced STM Index Map properties"
  [
    QC.testProperty
...
stmBalancedIndexUnitTests = testGroup "STM Map unit tests"
  [
...
prop_get_always_returns_inserted_elements = monadicIO $ do
  ...
  -- s <- run $ atomically $ M.dumpLine m
  -- run $ print $ "s = " ++ show s ++ ", k = " ++ show keys ++ ", s = " ++ show values
